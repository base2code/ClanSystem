...
public class ClanDemoteCommand implements SubCommand {
    @Override
        ...
        ClanSystem.getInstance().getClanManager().getClanOfUser(player.getUniqueId()).whenComplete((clan, throwable) -> {
            ...
            ClanSystem.getInstance().getClanManager().getClanOfUser(targetUUID).whenComplete((clan1, throwable1) -> {
                ...
                if (currentRole == ClanPermissionRole.OWNER) {
                    commandSender.sendMessage(ClanSystem.getInstance().getMessage("error.cant-demote-owner"));
                    return;
                } else if (currentRole == ClanPermissionRole.MODERATOR) {
                    commandSender.sendMessage(ClanSystem.getInstance().getMessage("commands.clan.demote.pending.moderator").replace("%player%", targetName));
                    Confirmation confirmation = new Confirmation(player) {
                        @Override
                        public void run() {
                            ClanSystem.getInstance().getClanManager().setPermissionRoleInClan(clan, targetUUID, ClanPermissionRole.MEMBER);
                            commandSender.sendMessage(ClanSystem.getInstance().getMessage("commands.clan.demote.success").replace("%player%", targetName));
                        }
                    };
                    ClanSystem.getInstance().getConfirmationManager().addConfirmation(confirmation);
                } else if (currentRole == ClanPermissionRole.MEMBER) {
                    commandSender.sendMessage(ClanSystem.getInstance().getMessage("error.cant-demote-member"));
                }
            });
        });
